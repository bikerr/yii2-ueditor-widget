...
//"imagePathFormat": "/ueditor/php/upload/image/{yyyy}{mm}{dd}/{time}{rand:6}", /* 上传保存路径,可以自定义保存路径和文件名格式 */
    ...
    private function actionCrawler($CONFIG) {
        try {
            if (is_callable($this->beforeCatch)) {
                call_user_func($this->beforeCatch, $this);
            }
        ...
        if (empty($source)){
            return Json::encode(['status'   =>  'source can not be empty']);
        }
        ...
        if ($this->result['state'] == 'SUCCESS') {
            try {
                if (is_callable($this->afterCatch)) {
                    call_user_func($this->afterCatch,$this);
                }
            } catch (\yii\base\Exception $e) {
                return Json::encode(['state' => $e->getMessage()]);
            }
